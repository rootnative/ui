...
- The anchor is annotated for you, in both the controlled and uncontrolled forms: `aria-haspopup="menu"` plus an `aria-expanded` that tracks the menu, so a screen reader announces the trigger as opening a menu rather than as a plain button.
- **On web**, focus moves into the menu when it opens, Arrow Down/Up and Tab move between items, Escape dismisses, and focus returns to the anchor on close. See [Accessibility](../accessibility.md#dialogs-menus-and-sheets-on-web). The surface is deliberately *not* marked modal on native — that would take the dismiss region below out of the accessibility tree along with it.
