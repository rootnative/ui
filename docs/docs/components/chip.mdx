---
sidebar_position: 4
description: Chips enter information, make selections, filter content, or trigger actions. Follows the Material Design 3 Chip specification.
---
...
Available on `assist`, `filter`, and `suggestion` variants. Input chips are always outlined, so `elevated` is not part of their props — TypeScript rejects `<Chip variant="input" elevated>` rather than ignoring it.
...
The close icon is a separate accessibility target from the chip itself, so it names itself `Remove {label}` rather than a bare "Remove" — a screen-reader user hears which chip they are about to remove. Override the whole string with `closeAccessibilityLabel` when localizing.
