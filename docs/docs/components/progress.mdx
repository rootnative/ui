---
sidebar_position: 6
description: Express an unspecified wait or show how far a process has run. Follows the Material Design 3 Progress Indicators specification.
---
...
The animations run on the UI thread through [`@rootnative/inertia`](../motion), so they stay smooth even while the JS thread is busy.
