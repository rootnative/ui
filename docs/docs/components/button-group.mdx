---
sidebar_position: 2
description: A row of connected or pill-shaped related actions, with single or multiple selection — the MD3 replacement for Segmented Button.
---
...
| `connected` | Buttons read as a single segmented bar, divided by a 2dp separator gap; the selected button morphs to a smaller corner radius |
...
| `single` | At most one item can be selected. Pressing the selected item deselects it, and `onValueChange` fires with `null` | `string \| null` |
