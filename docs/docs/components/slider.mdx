---
sidebar_position: 8
description: Select a value or a range from a continuous or discrete set. Follows the Material Design 3 Slider specification.
---
...
`containerColor` overrides the active track, `contentColor` overrides the thumb, and `inactiveTrackColor` overrides the inactive track. Ticks and stop indicators on the active segment stay `onPrimary` per MD3 and are not derived from these overrides. The MD3 disabled treatment is preserved regardless of overrides.
