...
- **On web**, the anchor carries `aria-describedby` pointing at the tooltip while it is shown, which
  is the only thing that connects the two — the surface renders in a portal, far from the anchor in
  the DOM. React Native has no equivalent, so this is web-only.
