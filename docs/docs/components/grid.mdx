---
sidebar_position: 4
description: A multi-column grid that wraps children into equal-width columns, built on Row.
---
...
<SnackExample
  name="RootNative Grid — Spanning Cells"
  code={`
  import { Card, Grid, Typography } from '@rootnative/components'
  import { ThemeProvider } from '@rootnative/core'
  import { SafeAreaProvider } from 'react-native-safe-area-context'
...
A `<Stagger>` from `@rootnative/inertia` can wrap the grid's children directly.
`Grid` looks through it, so each child still gets its own cell and still
inherits its own delay:

```tsx
import { Grid } from '@rootnative/components'
import { Motion, Stagger } from '@rootnative/inertia'

<Grid columns={{ compact: 1, medium: 2 }} gap="md">
  <Stagger interval={60}>
    {items.map((item) => (
      <Motion.View
        key={item.id}
...
Put the `Stagger` **inside** the `Grid`, not around it. A `Grid` wrapped in a
`Stagger` is a single child of that stagger, so every card shares one delay and
