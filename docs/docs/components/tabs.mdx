---
sidebar_position: 1
description: Organize content into groups the user switches between without leaving the screen.
---
...
indicator matches the width of the tab's *content* — the label, or the icon-and-label column when
an icon stacks above it — rather than the width of the tab (floored at 24dp so a one-character
label still gets something to see). A **secondary** indicator spans the whole tab.
